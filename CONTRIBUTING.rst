...
MTV could always use more documentation, whether as part of the
official MTV docs, in docstrings, or even on the web in blog posts,
articles, and such.
...
Ready to contribute? Here's how to set up `MTV` for local development.

1. Fork the `MTV` repo on GitHub.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/MTV.git
...
2. The tests that cover a module called ``MTV/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/MTV/path/to/test_a_module.py``.
...
To run a subset of tests::
    ...
    $ python -m pytest tests.test_MTV
...
2. Update the version in ``setup.cfg``, ``MTV/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``MTV/__init__.py``
...
.. _GitHub issues page: https://github.com/HDI-Project/MTV/issues
